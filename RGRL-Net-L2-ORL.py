...
def test_face(Img, Label, CAE, num_class,lr2=5e-4):
    ...
    for i in range(0,41-num_class): 
        ...
        #     CAE.save_model()
        #     pred = True
        max_step = 5000#50 + num_class*25# 100+num_class*20
        display_step = 10000#max_step/20#10
        lr = 1.0
        fine_step = -1
        # fine-tune network
        epoch = 0
...
if __name__ == '__main__':
    ...
    model_path = './pretrain-model-ORL/model-335-32x32-orltemp.ckpt'
    restore_path = './pretrain-model-ORL/model-335-32x32-orltemp.ckpt'
    logs_path = './logs'
    ...
    for reg2 in [1,2,5,10]:
        for reg3 in [1]:#[1e-7,1e-6,5e-6]:
            for reg4 in [0.01,0.1,0.5,1,2]:
                for lr2 in [1e-4]:
                    try:
